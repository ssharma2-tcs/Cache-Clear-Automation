...
(async function example() {
  ...
  try {
    ...
    async function cPCodeCacheClear(c, i) {
      let code = await driver.wait(until.elementLocated(By.css(`input[placeholder="Select CP codes (max 100)"]`)), 10000)
      await code.click();
      await code.clear();
      ...
      await driver.sleep(3000);
      let selectTd = await driver.wait(until.elementLocated(By.css(`td[class="cdk-cell cdk-column-cpCode akam-cell akam-column-cpCode ng-star-inserted"]`)), 10000)
      ...
      await driver.sleep(2000);
      let selectEnv = await driver.wait(until.elementLocated(By.css(`span[translate="template.ccu.networkType.prod"]`)), 10000)
      await selectEnv.click();
      await driver.sleep(2000);
      let selectCacheway = await driver.wait(until.elementLocated(By.css(`span[translate="template.ccu.refreshType.purge"]`)), 10000)
      await selectCacheway.click();
      await driver.sleep(3000);
      let cacheIt = await driver.wait(
          until.elementLocated(By.css(`button[translate="template.ccu.button.submit"]`)), 10000
      );
      await cacheIt.click();
      await driver.sleep(3000);
      selectTd.click();
